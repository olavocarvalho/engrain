...
describe('remove command', () => {
  ...
  test('throws error when doc does not exist', async () => {
    ...
    await expect(runRemoveCommand('nonexistent', { output: agentsFile })).rejects.toThrow();
  });

  test('throws error when file does not exist', async () => {
    // Try to remove from non-existent file
    await expect(runRemoveCommand('test-doc', { output: agentsFile })).rejects.toThrow();
  });
  ...
  test('updates lock file after removal', async () => {
    ...
    // Add to lock file (uses process.cwd() which is testDir)
    await addDocsToLock('test-doc', {
