...
describe('clear command', () => {
  ...
  test('throws error without --force in non-interactive mode', async () => {
    // Setup: Add a doc
    ...
    try {
      // Try to clear without force - should throw in non-interactive mode
      await expect(runClearCommand({ output: agentsFile, force: false })).rejects.toThrow(
  ...
  test('clears lock file entries for project', async () => {
    ...
    let lock = await getAllDocsForProject();
    ...
    lock = await getAllDocsForProject();
  ...
  test('only clears current project from lock file', async () => {
    ...
    try {
      ...
      const currentLock = await getAllDocsForProject();
